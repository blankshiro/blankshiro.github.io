...
$ sudo nmap -sn $IP/24 # disable port scan and enumerate only subnet
$ sudo nmap -p- --min-rate 1000 $IP -Pn # disables ping command and only scans ports 
...
$ sudo nmap -sS -p- --min-rate=1000 $IP -Pn # stealth scans
$ ncat -nv --source-port 53 $IP <port> # Connect To a Filtered Port
$ sudo nmap $IP/24 -sn -oA filename | grep for | cut -d" " -f5 # Scan network ranges
$ sudo nmap -sn -oA filename -iL hosts.lst | grep for | cut -d" " -f5 # Scan network ranges on a predefined ip list
...
$ crackmapexec smb $IP -u users.txt -p 'Pass!' --local-auth --continue-on-success
